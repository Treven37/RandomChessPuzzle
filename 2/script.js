		...
		var guessWord;
		if(Math.random() > 1/6) {
			guessWord = `012a012b012c`;
		} else if(Math.random() > 1/6) {
			guessWord = `012a012c012b`;
		} else if(Math.random() > 1/6) {
			guessWord = `012b012a012c`;
		} else if(Math.random() > 1/6) {
			guessWord = `012b012c012a`;
		} else if(Math.random() > 1/6) {
			guessWord = `012c012a012b`;
		} else {
			guessWord = `012c012b012a`;
		}
		/*
		if(prob==0) {
			e = a+b+c-d;
			...
			if(removed.includes(x+' '+y)) {
				...
				let letterJump = setInterval(function() {
					if(document.querySelectorAll('.white, .black, .removed')[pos].innerHTML == ""){
						document.querySelectorAll('.white, .black, .removed')[pos].innerHTML = guessWord[guessWord.length-1 - countSquare] === " "?"&":guessWord[guessWord.length-1 - countSquare];
						countSquare++;
